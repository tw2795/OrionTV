...
const useDetailStore = create<DetailState>((set, get) => {
  const resolutionFetchQueue = new Map<string, SearchResultWithResolution>();
  let isProcessingResolutionQueue = false;

  const processResolutionQueue = async () => {
    while (resolutionFetchQueue.size > 0) {
      const controller = get().controller;
      const signal = controller?.signal;
      if (signal?.aborted) {
        resolutionFetchQueue.clear();
        break;
      }

      const iterator = resolutionFetchQueue.entries().next();
      if (iterator.done) {
        break;
      }
      const [key, item] = iterator.value;
      resolutionFetchQueue.delete(key);

      if (!item || item.resolution || !item.episodes || item.episodes.length === 0) {
        continue;
      }
      ...
      try {
        ...
        logger.info(`Background resolution fetch failed for ${item.source_name}`, error);
      }
      ...
      const activeController = get().controller;
      ...
      set((state) => {
        const updateSearchResults = state.searchResults.map((entry) =>
          entry.source === item.source && entry.id === item.id ? { ...entry, resolution } : entry,
        );
        const updateSources = state.sources.map((entry) =>
          entry.source === item.source ? { ...entry, resolution } : entry,
        );
        return {
    ...
    isProcessingResolutionQueue = false;
  };

  const scheduleResolutionFetch = (items: SearchResultWithResolution[]) => {
    if (items.length === 0) {
      return;
    }
    items.forEach((item) => {
      if (!item || item.resolution) return;
      const key = `${item.source}-${item.id}`;
      if (!resolutionFetchQueue.has(key)) {
        resolutionFetchQueue.set(key, item);
      }
    });

    if (!isProcessingResolutionQueue && resolutionFetchQueue.size > 0) {
      isProcessingResolutionQueue = true;
      setTimeout(() => {
  ...
  return {
    ...
    init: async (q, preferredSource, id) => {
      ...
      const processAndSetResults = async (results: SearchResult[], merge = false) => {
        const perfStart = performance.now();
        logger.info(`[PERF] Process search results START - processing ${results.length} sources`);

        if (signal.aborted) return;

        const resultsWithMetadata: SearchResultWithResolution[] = results.map((searchResult) => ({
          ...searchResult,
          resolution: (searchResult as SearchResultWithResolution).resolution ?? null,
        }));

        let newResults: SearchResultWithResolution[] = [];
        let finalResults: SearchResultWithResolution[] = [];

        set((state) => {
          const existingKeys = new Set(state.searchResults.map((r) => `${r.source}-${r.id}`));

          if (merge) {
            newResults = resultsWithMetadata.filter((r) => !existingKeys.has(`${r.source}-${r.id}`));
            finalResults = [...state.searchResults, ...newResults];
          } else {
            newResults = resultsWithMetadata;
            finalResults = resultsWithMetadata;
          }
        ...
        logger.info(`[PERF] Process search results COMPLETE - took ${(perfEnd - perfStart).toFixed(2)}ms`);

        if (!signal.aborted) {
          scheduleResolutionFetch(newResults);
        }
          ...
          if (preferredResult.length > 0) {
            ...
            try {
              ...
              if (filteredResults.length > 0) {
                logger.info(
                  `[SUCCESS] FALLBACK search found results, proceeding with ${filteredResults[0].source_name}`,
                );
    ...
    getNextAvailableSource: (currentSource: string, episodeIndex: number) => {
      ...
      logger.info(
        `[SOURCE_SELECTION] Selected fallback source: ${selectedSource.source} (${selectedSource.source_name}) with resolution: ${selectedSource.resolution || "unknown"}`,
      );
